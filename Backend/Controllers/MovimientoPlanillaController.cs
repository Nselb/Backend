...
namespace Backend.Controllers
{
    ...
    public class MovimientoPlanillaController : ControllerBase
    {
        ...
        public async Task<MovimientoPlanilla?> Update([FromBody] MovimientoPlanilla cc)
        {
            using var client = new HttpClient();
            client.BaseAddress = new Uri(uri);
            var responseTask = client.GetAsync($"MovimientoPlanillaUpdate?conceptos={cc.Concepto}&prioridad={cc.Prioridad}&tipooperacion={cc.TipoOperacion}&cuenta1={cc.Cuenta1}&cuenta2={cc.Cuenta2}&cuenta3={cc.Cuenta3}&cuenta4={cc.Cuenta4}&MovimientoExcepcion1={cc.MovimientoExcepcion1}&MovimientoExcepcion2={cc.MovimientoExcepcion2}&MovimientoExcepcion3={cc.MovimientoExcepcion3}&Traba_Aplica_iess={cc.Aplica_iess}&Aplica_Proy_Renta={cc.Aplica_imp_renta}&Empresa_Afecta_Iess={cc.Empresa_Afecta_Iess}");
            var result = responseTask.Result;
            MovimientoPlanilla? centro = new();
            if (result.IsSuccessStatusCode)
            {
        ...
        public async Task<MovimientoPlanilla?> Delete(int codigo, string descripcion)
        {
            using var client = new HttpClient();
            client.BaseAddress = new Uri(uri);
            var responseTask = client.GetAsync($"MovimientoPlanillaDelete?codigomovimiento={codigo}&descripcionomovimiento={descripcion}");
            var result = responseTask.Result;
            MovimientoPlanilla? centro = new();
            if (result.IsSuccessStatusCode)
            {
